		...
		if ($file = @fopen($filename, 'r')) {
			...
			while ($row = fgetcsv($file, $options['length'], $options['delimiter'], $options['enclosure'])) {
				...
				foreach ($fields as $f => $field) {
					if(isset($row[$f])){
						$possibleEncodingArray = array("ASCII", "JIS", "EUC-JP", "SJIS", "auto");
						if($possibleEncoding=mb_detect_encoding($row[$f],$possibleEncodingArray)){
							$row[$f]=mb_convert_encoding($row[$f], 'UTF-8',$possibleEncoding);
						} else {
